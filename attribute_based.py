    ...
    for i, (data, target) in enumerate(tqdm(dataloader)):
        ...
        if len(attribute) > 1:
            ...
            for a in range(len(categories)):
                ...
                counts[attribute[0] - 1]["{0}-{1}".format(cat_a, cat_a)] += 1
                for b in range(a+1, len(categories)):
                    cat_b = dataloader.dataset.categories.index(categories[b])
                    if cat_a < cat_b:
                        counts[attribute[0] - 1]["{0}-{1}".format(cat_a, cat_b)] += 1
                    else:
                        counts[attribute[0] - 1]["{0}-{1}".format(cat_b, cat_a)] += 1
...
def distance_for_instance(dataloader, args):
    ...
    for i, (data, target) in enumerate(tqdm(dataloader)):
        ...
        file_path = target[3]
        if len(attribute) > 1:
            person_center = np.array([attribute[1][0] + (attribute[1][1]/2.), attribute[1][2] + (attribute[1][3]/2.)])
            person_area = (attribute[1][1]-attribute[1][0])*(attribute[1][3]-attribute[1][2])
            seen_instances = []
            for j, ann in enumerate(anns):
                ...
                if categories.index(ann['label']) in seen_instances:
                    prev = distances[categories.index(ann['label'])][attribute[0] - 1][-1]
                    prev_calc_dist = prev[0] / np.sqrt(prev[1]*prev[2])
                    calc_dist = distance / np.sqrt(person_area*ann_area)
                    if calc_dist < prev_calc_dist:
                        distances[categories.index(ann['label'])][attribute[0] - 1][-1] = (distance, person_area, ann_area, file_path, j)
                else:
                    distances[categories.index(ann['label'])][attribute[0] - 1].append((distance, person_area, ann_area, file_path, j))
    ...
    for i, (data, target) in enumerate(tqdm(dataloader)):
        ...
        if len(attribute) > 1:
            ...
            for ann in anns:
                ...
                if len(scene_features[index][attribute[0]-1]) < 500 and index not in scene_added:
                    scene_added.append(index)
                    scene_features[index][attribute[0]-1].extend(this_features.data.cpu().numpy())
                    scene_filepaths[index][attribute[0]-1].append((target[3], pred))
                if len(instance_features[index][attribute[0]-1]) < 500:
                    instance_features[index][attribute[0]-1].extend(this_small_features.data.cpu().numpy())
    stats = {}
    ...
    for i, (data, target) in enumerate(tqdm(dataloader)):
        ...
        if len(attribute) > 1:
            for scene in top_scene:
                scenes_per[scene][attribute[0]-1] += 1
