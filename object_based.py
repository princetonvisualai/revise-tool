...
def count_cooccurrence(dataloader, args):
    ...
    groupings_size = [[] for i in range(len(GROUPINGS_TO_NAMES))]
    groupings_dist = [[] for i in range(len(GROUPINGS_TO_NAMES))]
    group_mapping = dataloader.dataset.group_mapping
    img_center = np.array([.5, .5])
    instances_size = [[] for i in range(len(all_categories))]

    filepaths = [[] for i in range(len(GROUPINGS_TO_NAMES))] # for qualitative examples in analysis step

    with_people_instances = np.zeros(len(all_categories))
    with_people = np.zeros(len(GROUPINGS_TO_NAMES))
    not_with_people = np.zeros(len(GROUPINGS_TO_NAMES))
...
def scene_categorization(dataloader, args):
    ...
    scene_supercategory = np.zeros((len(idx_to_scenegroup), len(GROUPINGS_TO_NAMES)))
    ...
    for i in range(len(GROUPINGS_TO_NAMES)):
